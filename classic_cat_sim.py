...
class Simulation():
    ...
    def execute_rotation(self, time, next_tick):
        ...
        if self.player.mana < self.player.shift_cost:
            ...
            elif (energy >= 48) or self.player.omen_proc:
                return self.player.shred()
        elif energy < 10:
            ...
            if (energy >= 63) or ((energy >= 15) and self.player.omen_proc):
                return self.player.shred()
            if energy >= 35:
                return self.player.bite()
            ...
            # can Shred afterward, and otherwise shift and won't Bite
            ...
            if ((energy >= 28) and bite_before_rip
            ...
            if (energy >= 48) or self.player.omen_proc:
                return self.player.shred()
            if ((energy >= 40) and (time_to_next_tick > 1.0)
                    and self.strategy['use_claw_trick']):
                return self.player.claw()
    ...
    def run(self, log=False):
        ...
        self.player.reset()
        self.innervate_threshold = 2 * self.player.shift_cost + 62
        ...
        # accurate to a given run, as it does not incorporate partial Rip ticks
        # at the end. So we'll keep track of it ourselves.
        rip_damage = 0.0
        ...
        while time <= self.fight_length:
            ...
            if self.mcp_equipped and mcp_active and (time > mcp_end - 1e-9):
                mcp_active = False
                self.mcp_equipped = False
                self.update_swing_times(time, self.swing_timer * 1.5)
        ...
        # Replace logged Rip damage with the actual value realized in the run
    ...
    def calc_stat_weights(self, num_replicates, base_dps=None):
        ...
        # For AP, we will use an increment of +80 AP
        ...
        swing_delta = self.player.swing_timer * 0.04 / 1.04
    ...
    def calc_mana_weights(self, num_replicates, base_dps, dps_per_AP):
        ...
        dps_deltas['1 Int'] = 15. / self.player.shift_cost * self.calc_deriv(
        ...
        spirit_delta = base_regen_delta * 5
        ...
        for stat in ['1 Int', '1 Spirit', '1 mp5']:
